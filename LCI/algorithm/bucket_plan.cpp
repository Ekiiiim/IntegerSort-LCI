...
BucketPlan build_bucket_plan(const KeyCount* local_bucket_counts, const KeyCount* global_bucket_counts,
  ...
  KeyRank bucket_sum_accumulator = 0;
  KeyCount expected_recv_count = 0;
  KeyRank previous_bucket_sum_accumulator = 0;
  int owner_rank = 0;
  ...
  for (int bucket = 0; bucket < num_buckets; bucket++) {
    ...
    if (bucket_sum_accumulator >= static_cast<KeyRank>(owner_rank + 1) * average_keys_per_rank) {
      if (owner_rank == my_rank) {
        expected_recv_count = static_cast<KeyCount>(bucket_sum_accumulator - previous_bucket_sum_accumulator);
      }
      if (owner_rank != 0) {
        first_bucket_by_rank[owner_rank] = last_bucket_by_rank[owner_rank - 1] + 1;
      }
  ...
  const KeyValue min_key_value = first_bucket_by_rank[my_rank] << bucket_shift;
  const KeyValue max_key_value = ((last_bucket_by_rank[my_rank] + 1) << bucket_shift) - 1;

  KeyRank lesser_key_count = 0;
  for (int rank = 0; rank < my_rank; rank++) {
    for (int bucket = first_bucket_by_rank[rank]; bucket <= last_bucket_by_rank[rank]; bucket++) {
      lesser_key_count += global_bucket_counts[bucket];
    }
  }

  KeyCount local_key_count = 0;
  for (int bucket = first_bucket_by_rank[my_rank]; bucket <= last_bucket_by_rank[my_rank]; bucket++) {
    local_key_count += global_bucket_counts[bucket];
  }

  return BucketPlan{
      expected_recv_count, min_key_value, max_key_value, lesser_key_count, local_key_count,
  };
}
