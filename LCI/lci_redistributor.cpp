...
constexpr size_t KEY_SIZE = sizeof(Count);
...
void handle_received_keys(const void* src, size_t num_keys) {
  const Count* keys = static_cast<const Count*>(src);
  for (size_t i = 0; i < num_keys; ++i) {
    active_runtime->frequency_histogram[keys[i]].fetch_add(
...
void* get_upacket_blocking(lci::device_t device,
                           int dest_rank,
                           int comm_size,
  ...
  const int buffer_index =
      (thread_id * comm_size + dest_rank) * options.message_batch_size;
  return static_cast<void*>(
      static_cast<Count*>(runtime->fallback_buffer) + buffer_index);
}
...
class SendBuffer {
 public:
  ...
  void push(Count key,
    ...
    if (buffer_ == nullptr) {
      buffer_ =
          get_upacket_blocking(device, dest_rank, comm_size, options, runtime);
      values_ = static_cast<Count*>(buffer_);
    }
    values_[size_++] = key;
  }
 ...
 private:
  ...
  Count* values_ = nullptr;
  int size_ = 0;
};

void flush_send_buffer(std::vector<SendBuffer>& send_buffers,
                       int dest_rank,
                       int my_rank,
                       lci::device_t device,
                       const RedistributorOptions& options,
  ...
  if (dest_rank == my_rank && options.use_loopback) {
    ...
    lci::status_t status;
    do {
      status = lci::post_am_x(dest_rank,
                              send_buffer.data(),
                              send_buffer.size_in_bytes(),
                              runtime->send_counter,
                              runtime->rcomp)
...
void send_key_to_processor(Count key,
                           int dest_rank,
                           ...
                           const RedistributorOptions& options,
                           RedistributorRuntime* runtime) {
  send_buffers[dest_rank].push(
      key, device, dest_rank, comm_size, options, runtime);
  if (send_buffers[dest_rank].size() >=
      static_cast<size_t>(options.message_batch_size)) {
    flush_send_buffer(
        send_buffers, dest_rank, my_rank, device, options, runtime);
  }
}

void flush_all_send_buffers(std::vector<SendBuffer>& send_buffers,
                            int comm_size,
                            int my_rank,
                            lci::device_t device,
                            const RedistributorOptions& options,
  ...
  for (int i = 0; i < comm_size; ++i) {
    ...
    if (!send_buffers[index].empty()) {
      flush_send_buffer(
          send_buffers, index, my_rank, device, options, runtime);
    }
  }
}
...
void initialize_redistributor(RedistributorRuntime* runtime,
  ...
  runtime->handler =
      lci::alloc_handler_x(am_handler).zero_copy_am(options.use_upacket)();
...
void allocate_fallback_buffers(RedistributorRuntime* runtime,
  ...
  runtime->fallback_buffer_bytes =
      static_cast<size_t>(options.message_batch_size) * sizeof(Count) *
...
#pragma omp for nowait
    ...
    flush_all_send_buffers(
        send_buffers, comm_size, my_rank, device, options, runtime);

#ifdef A2A_TL_TIMERS
    TL_STEP_START(A2A_PROGRESS_WAIT);
#endif
    while (runtime->received_count.load(std::memory_order_relaxed) <
           static_cast<size_t>(expected_recv_count)) {
      lci::progress_x().device(device)();
    }
#ifdef A2A_TL_TIMERS
