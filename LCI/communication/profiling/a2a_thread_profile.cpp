...
static inline uint64_t now_ns() {
  return (uint64_t)std::chrono::duration_cast<std::chrono::nanoseconds>(clock_steady::now().time_since_epoch()).count();
}
...
const char* step_name(Step s) {
  static const char* names[A2A_NUM_STEPS] = {"flush_send", "progress_wait", "am_copy", "self_copy"};
...
void print_per_thread(int iteration, int rank) {
  ...
  for (int t = 0; t < (int)stats.size(); ++t) {
    for (int s = 0; s < A2A_NUM_STEPS; ++s) {
      ...
      std::printf("  %6d  %-12s %8llu  %12llu  %10.3f\n", t, step_name((Step)s), (unsigned long long)stats[t].calls[s],
                  (unsigned long long)stats[t].bytes[s], ms);
    }
  }
}
...
void print_minmax(int iteration, int rank) {
  ...
  for (int s = 0; s < A2A_NUM_STEPS; ++s) {
    ...
    std::printf("  %-12s %-10s %12llu (t=%d)   %12llu (t=%d)\n", step_name((Step)s), "calls",
                (unsigned long long)calls.min_v, calls.tmin, (unsigned long long)calls.max_v, calls.tmax);

    std::printf("  %-12s %-10s %12llu (t=%d)   %12llu (t=%d)\n", step_name((Step)s), "bytes",
                (unsigned long long)bytes.min_v, bytes.tmin, (unsigned long long)bytes.max_v, bytes.tmax);

    std::printf("  %-12s %-10s %12.3f (t=%d)   %12.3f (t=%d)\n", step_name((Step)s), "time_ms", time.min_v, time.tmin,
